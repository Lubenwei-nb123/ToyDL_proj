...
class Variable:
    ...
    def __matmul__(self, other):
        try:
            return toydl.functions.matmul(self, other)
        except TypeError:
            return NotImplementedError
    ...
    def backward(self, *, retain_grad=False, create_graph=False):
        ...
        while funcs:
            ...
            with using_config(Config, 'enable_backprop', create_graph):
                gxs = f.backward(*gys)
    ...
    def reshape(self, *shape):
        if len(shape) == 1 and isinstance(shape[0], (tuple, list)):
            shape = shape[0]
        return toydl.functions.reshape(self, shape)

    def transpose(self):
        return toydl.functions.transpose(self)
    ...
    def sum(self, axis=None, keepdims=False):
        return toydl.functions.sum(self, axis, keepdims)
...
class Exp(Function):
    ...
    @Function.tuplize
    def backward(self, gy, /):
        return np.exp(self._inputs[0]) * gy
...
class Add(Function):
    ...
    def backward(self, gy, /):
        ...
        if x0.shape != x1.shape:
            gx0 = toydl.functions.sum_to(gy, x0.shape)
            gx1 = toydl.functions.sum_to(gy, x1.shape)
        return gx0, gx1
...
class Mul(Function):
    ...
    def backward(self, gy, /):
        ...
        gx0, gx1 = x1 * gy, x0 * gy
        if x0.shape != x1.shape:
            gx0 = toydl.functions.sum_to(gx0, x0.shape)
            gx1 = toydl.functions.sum_to(gx1, x1.shape)
        return gx0, gx1
...
class Pow(Function):
    ...
    def backward(self, gy, /):
        x0, x1 = self.inputs
        gx0, gx1 = gy * c * (x ** (c - 1)), gy * (x ** c) * log(x)
        if x0.shape != x1.shape:
            gx0 = toydl.functions.sum_to(gx0, x0.shape)
            gx1 = toydl.functions.sum_to(gx1, x1.shape)
        return gx0, gx1
...
def log(x, /) -> list[Variable] | Variable:
    return Log()(x)
